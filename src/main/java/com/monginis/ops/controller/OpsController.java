...
public class OpsController {
	...
	List<Item> posItemsList = new ArrayList<>();
	...
	public String newcustomerbill(@PathVariable int type, HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			// ItemResponse itemsList = restTemplate.postForObject(Constant.URL +
			// "/getItemsNameByIdWithOtherItem", mvm,
			// ItemResponse.class);
			ItemResponse itemsList = restTemplate.postForObject(Constant.URL + "/getAllItemsForPos", mvm,
					ItemResponse.class);

			List<Item> newItemsList = itemsList.getItemList();
			posItemsList = new ArrayList<>();
			posItemsList = itemsList.getItemList();
	...
	public AddCustemerResponse submitEditCustomer(HttpServletRequest request, HttpServletResponse responsel) {
		...
		try {
			...
			CustomerForOps[] customer = restTemplate.getForObject(Constant.URL + "/getAllCustomers",
					CustomerForOps[].class);
			List<CustomerForOps> customerList = new ArrayList<>(Arrays.asList(customer));
	 ...
	 * 
	 * @ResponseBody public Info submitBill(HttpServletRequest request,
	 * HttpServletResponse responsel) {
	 * 
	 * Info info = new Info(); RestTemplate restTemplate = new RestTemplate();
	 * 
	 * try {
	 * 
	 * 
	 ...
	 * 
	 * int index = Integer.parseInt(request.getParameter("key")); int custId =
	 * Integer.parseInt(request.getParameter("custId")); int advKey =
	 * Integer.parseInt(request.getParameter("advKey")); String customerName =
	 * request.getParameter("selectedText"); float advAmt =
	 * Float.parseFloat(request.getParameter("advAmt")); String advOrderDate =
	 * request.getParameter("advOrderDate"); int isAdvanceOrder =
	 * Integer.parseInt(request.getParameter("isAdvanceOrder"));
	 * 
	 * System.err.println("advAmt" + advAmt); HttpSession session =
	 * request.getSession(); Franchisee frDetails = (Franchisee)
	 * session.getAttribute("frDetails"); FrEmpMaster frEmpDetails = (FrEmpMaster)
	 * session.getAttribute("frEmpDetails");
	 * 
	 * String items = "0"; for (int i = 0; i < itemBillList.size(); i++) { items =
	 * items + "," + itemBillList.get(i).getItemId(); } MultiValueMap<String,
	 * Object> map = new LinkedMultiValueMap<>(); map.add("custId", custId);
	 * Customer customerById = restTemplate.postForObject(Constant.URL +
	 * "/getCustomerByCustId", map, Customer.class);
	 * 
	 * MultiValueMap<String, Object> mvm = new LinkedMultiValueMap<String,
	 * Object>(); mvm.add("itemList", items); ItemResponse itemResponse =
	 * restTemplate.postForObject(Constant.URL + "/getItemsById", mvm,
	 * ItemResponse.class); List<Item> itemsListByIds = itemResponse.getItemList();
	 * 
	 * List<SellBillDetail> sellbilldetaillist = new ArrayList<>();
	 * 
	 * float total = 0; float taxableAmt = 0; float taxAmt = 0;
	 * 
	 * for (int i = 0; i < itemBillList.size(); i++) {
	 * 
	 ...
	 * itemBillList.get(i).getTaxPer()); sellBillDetail.setMrpBaseRate(mrpBaseRate);
	 * 
	 ...
	 * 
	 * System.err.println("ITEM ADD -------------------- " +
	 * itemsListByIds.get(j).getExtVar2());
	 * 
	 * sellBillDetail.setExtVar1(itemsListByIds.get(j).getItemImage());
	 * sellbilldetaillist.add(sellBillDetail); total = total +
	 * sellBillDetail.getGrandTotal(); taxableAmt = taxableAmt +
	 * sellBillDetail.getTaxableAmt(); taxAmt = taxAmt +
	 * sellBillDetail.getTotalTax();
	 * 
	 * break; } }
	 * 
	 * }
	 * 
	 * SimpleDateFormat sf = new SimpleDateFormat("yyyy-MM-dd"); Date date = new
	 * Date();
	 * 
	 * SellBillHeader sellBillHeader = new SellBillHeader();
	 * 
	 ...
	 * frDetails.getFrId()); PettyCashManagmt petty =
	 * restTemplate.postForObject(Constant.URL + "/getPettyCashDetails", map,
	 * PettyCashManagmt.class);
	 * 
	 * String billDate = sf.format(date); if (petty != null) {
	 * 
	 * SimpleDateFormat ymdSDF = new SimpleDateFormat("yyyy-MM-dd"); Calendar cal =
	 * Calendar.getInstance(); cal.setTimeInMillis(Long.parseLong(petty.getDate()));
	 * cal.add(Calendar.DAY_OF_MONTH, 1);
	 * 
	 * billDate = ymdSDF.format(cal.getTime());
	 * System.err.println("BILL DATE ---------------- " + billDate); }
	 * 
	 * sellBillHeader.setBillDate(billDate); sellBillHeader.setCustId(custId);
	 * sellBillHeader.setInvoiceNo(getInvoiceNo(request, responsel));
	 * sellBillHeader.setPaidAmt(Math.round(total));
	 * 
	 * sellBillHeader.setPaymentMode(1); sellBillHeader.setSellBillNo(0);
	 * sellBillHeader.setUserGstNo(customerById.getGstNo());
	 * sellBillHeader.setUserPhone(customerById.getPhoneNumber());
	 * sellBillHeader.setBillType('R'); sellBillHeader.setTaxableAmt(taxableAmt);
	 * sellBillHeader.setDiscountPer(0); sellBillHeader.setDiscountAmt(0);
	 * 
	 ...
	 * 
	 * sellBillHeader.setRemainingAmt(0); sellBillHeader.setStatus(2);
	 * sellBillHeader.setSellBillDetailsList(sellbilldetaillist);
	 * sellBillHeader.setExtInt1(frEmpDetails.getFrEmpId());
	 * 
	 * float roundOff = 0; roundOff = taxableAmt + taxAmt - Math.round(total);
	 * sellBillHeader.setExtFloat1(roundOff);
	 * 
	 * info.setError(false); info.setMessage("Bill Submited");
	 * 
	 * hashMap.remove(index); itemBillList = new ArrayList<>(); SimpleDateFormat sf1
	 * = new SimpleDateFormat("dd-MM-yyyy"); SellBillHeader sellBillHeaderRes =
	 * restTemplate.postForObject(Constant.URL + "insertSellBillData",
	 ...
	 * 
	 * if (advAmt > 0) { transactionDetail.setCashAmt(Math.round(total - advAmt));
	 * transactionDetail.setExInt2(1);
	 * 
	 * } else { transactionDetail.setCashAmt(Math.round(total));
	 ...
	 * dList.add(transactionDetail); if (advAmt > 0) { transactionDetail = new
	 * TransactionDetail(); transactionDetail.setCashAmt(advAmt);
	 * transactionDetail.setPayMode(1);
	 * transactionDetail.setSellBillNo(sellBillHeaderRes.getSellBillNo());
	 ...
	 * .postForObject(Constant.URL + "saveTransactionDetail", dList,
	 * TransactionDetail[].class);
	 * 
	 * if (transactionDetailRes.length > 0) { transactionDetail = null; } map = new
	 ...
	 * 
	 * Info infores = restTemplate.postForObject(Constant.URL +
	 ...
	 * }
	 * 
	 * } info.setMessage(String.valueOf(sellBillHeaderRes.getSellBillNo())); } catch
	 * (Exception e) { e.printStackTrace(); info.setError(true);
	...
	public List<Item> getItemListByCatSubCatForCustomerBill(HttpServletRequest request, HttpServletResponse responsel) {
		...
		try {
			...
			newItemsList = itemsList.getItemList();
			System.err.println("NEW ITEM LIST - " + newItemsList);
	...
	@RequestMapping(value = "/getItemListByCatForPos", method = RequestMethod.POST)
	@ResponseBody
	public List<Item> getItemListByCatForPos(HttpServletRequest request, HttpServletResponse responsel) {
		...
		try {
			...
			ItemResponse itemsList = restTemplate.postForObject(Constant.URL + "/getItemsNameByCatIdForPos", mvm,
					ItemResponse.class);
			newItemsList = itemsList.getItemList();
			System.err.println("NEW ITEM LIST - " + newItemsList);
...
//				List<Item> newItemsList = itemsList.getItemList();
			...
			try {
				...
				TransactionDetail transactionDetail = restTemplate.postForObject(Constant.URL + "/getAdvAmtTransaction",
	...
	@RequestMapping(value = "/getItemDataByIdPos", method = RequestMethod.POST)
	@ResponseBody
	public Item getItemDataByIdPos(HttpServletRequest request, HttpServletResponse responsel) {
		...
		try {
			...
			if (posItemsList != null) {
				...
				for (int i = 0; i < posItemsList.size(); i++) {

					if (id == posItemsList.get(i).getId()) {
						item = posItemsList.get(i);
						break;
					}
